...
function drawNumber(count,hrztArr,vtcArr) {
    ...
    for(let i = 0; i < vtcArr.length; i++) {
        if (vtcArr[i][0] == count) {
            final = ' style = "background-image:url(img/blue' + vtcArr[i][1] + '.png)';
            break;
        }
    }
    for(let i = 0; i < hrztArr.length; i++) {
        if(final == "") {
            if (hrztArr[i][0] == count) {
                final = ' style = "background-image:url(img/red' + hrztArr[i][1] + '.png)';
                break;
            }
        }
        else if (hrztArr[i][0] == count) {
            final += ', url(img/red' + hrztArr[i][1] + '.png);';
            break;
        }
    }
    final += '"';
